...
Runs 100% locally for detection. Optional "Safe Cloud AI" tab sends only anonymized text to Claude.
"""
...
<style>
    ...
    .pill { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 0.8rem; font-weight: 600; margin-right: 6px; }
    .pill-local { background: #d4f4dd; color: #1e6b3a; }
    .pill-cloud { background: #fde4d0; color: #8a4a1c; }
    .stat-box { background: #f4f4f6; padding: 1rem; border-radius: 8px; text-align: center; }
    .stat-num { font-size: 2rem; font-weight: 700; color: #c15f3c; }
    .stat-label { font-size: 0.85rem; color: #666; }
    .mapping-row { background: #fff; border: 1px solid #e5e5e5; padding: 0.5rem 0.75rem; border-radius: 6px; margin-bottom: 0.4rem; display: flex; justify-content: space-between; align-items: center; }
    .original { color: #c1432c; font-family: monospace; }
    .replacement { color: #2c7a3c; font-family: monospace; font-weight: 600; }
    .type-badge { font-size: 0.7rem; background: #eee; padding: 2px 8px; border-radius: 10px; color: #555; }
    mark.hl { background: #fff3a8; padding: 0 3px; border-radius: 3px; }
</style>
""", unsafe_allow_html=True)

# ---------- HEADER ----------
st.markdown('<p class="big-title">🔒 Private AI Preprocessor</p>', unsafe_allow_html=True)
st.markdown('<p class="subtitle">Anonymize confidential documents <b>locally</b>, then safely use any cloud AI — without leaking names, dollar amounts, or addresses.</p>', unsafe_allow_html=True)
st.markdown('<span class="pill pill-local">✅ Detection runs locally</span><span class="pill pill-cloud">☁️ Cloud AI only sees anonymized text</span>', unsafe_allow_html=True)
st.markdown("---")
...
@st.cache_resource
def load_analyzer():
    """Load Presidio analyzer. Downloads spaCy model on first run if needed."""
    try:
    ...
    analyzer.registry.add_recognizer(PatternRecognizer(supported_entity="MONEY", patterns=[money_pattern]))

    postal_pattern = Pattern(name="ca_postal", regex=r"\b[A-Z]\d[A-Z]\s?\d[A-Z]\d\b", score=0.9)
    analyzer.registry.add_recognizer(PatternRecognizer(supported_entity="POSTAL_CODE", patterns=[postal_pattern]))
    ...
    analyzer.registry.add_recognizer(PatternRecognizer(supported_entity="STREET_ADDRESS", patterns=[street_pattern]))
...
def apply_mapping(text, analyzer_results, mapping):
    ...
    for r in results_sorted:
        ...
        applied.append({"original": original_text, "replacement": replacement, "type": r.entity_type})
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("🔒 Detection runs locally. Data only leaves your laptop if you use the Safe Cloud AI tab.")
...
with tab1:
    ...
    with col2:
        ...
        if go:
            if not text_input.strip():
                ...
                with st.spinner("Analyzing locally (first run downloads model, ~30s)..."):
        ...
        if "anonymized" in st.session_state:
            st.text_area("Anonymized", value=st.session_state["anonymized"], height=280, key="output")
    ...
    if "mappings" in st.session_state and st.session_state["mappings"]:
        ...
        with d2:
            decoder_payload = {"created_at": datetime.now().isoformat(), "mappings": mappings}
            st.download_button(
...
with tab3:
    ...
    st.markdown(
        ...
        "get back a response, then un-redact it locally. "
    ...
    if "anonymized" not in st.session_state:
        ...
        if task == "Custom prompt...":
            instruction = st.text_input("Your instruction", value="Summarize this document.")
        else:
            instruction = task
        ...
        if send:
            if not api_key:
                ...
                try:
                    ...
                    st.session_state["recovered_response"] = deanonymize(cloud_response, st.session_state["mappings"])
                except ImportError:
        ...
        if "cloud_response" in st.session_state:
            ...
            st.info("🔒 At no point did Claude see the original names, addresses, or dollar amounts. The decoder ring stayed on this machine.")
